...
public:
    ...
    KdNode(std::vector<Pack> l, Range r, Vector3f biggest)
    {
    ...
    void KdNode::split(std::vector<Pack> list_x, std::vector<Pack> list_y, std::vector<Pack> list_z, Vector3f biggest)
    {
        ...
        {
            KdNode child(tmp, left, biggest);
            children.push_back(child);
            if (flag)
            {
                KdNode child2(tm3, right, biggest);
                children.push_back(child2);
            }
            else
            {
                KdNode child2(tm2, right, biggest);
                children.push_back(child2);
            }
        }
    }
    ...
    std::pair<float, Triangle> findIntersection(Vector3f start, Vector3f dir,SurfaceInteraction &interaction,const Ray& ray)
    {
        if (children.empty())
        {
            ...
            for (long i = 0; i < triangles_size; ++i) {
                if (list[i].intersect(ray, temp) &&(temp.p.x <range.x_max && temp.p.x>range.x_min)&&(temp.p.y <range.y_max && temp.p.y>range.y_min)&&(temp.p.z <range.z_max && temp.p.z>range.z_min)) {
                    hit_tri = true;
                    tri_idx = i;
                    ray.t_max = temp.t;
                    interaction = temp;
            ...
            if(hit_tri == true)
            {
                std::pair<float, Triangle> p(1,list[tri_idx]);
                return p;
            }
            std::pair<float, Triangle> p(-1,list[0]);
            return p;
            //zwrocic taki zbior by wiadomo bylo ze do interakcji nie doszlo
        }
        else
        {
            ...
            if (withChild1)
            {
                if (withChild2)
                {
                    ...
                    {
                        if (children[0].findIntersection(start, dir,interaction,ray).first > 0)
                        {
                            return children[0].findIntersection(start, dir,interaction,ray);
                        }
                        else
                        {
                            return children[1].findIntersection(start, dir,interaction,ray);
                        }
                    }
                    else
                    {
                        if (children[1].findIntersection(start, dir,interaction,ray).first > 0)
                        {
                            return children[1].findIntersection(start, dir,interaction,ray);
                        }
                        else
